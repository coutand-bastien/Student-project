 ...
 */
bool check(const set<automaton>& automata);
 ...
 */
void generate_c_file(ostream& out, const set<automaton>& automata);
...
/*    // START TEST DETERMINISTIC :

    automaton automata;
    
    **************** automate 1 ****************
    automata.initials |= 0; automata.finals |= 4;
    automata.transitions |= transition(0, 'a', 1);
    automata.transitions |= transition(1,  2);
    automata.transitions |= transition(1,  4);
    automata.transitions |= transition(2, 'a', 3);
    automata.transitions |= transition(2, '0', 3);
    automata.transitions |= transition(3,  2);
    automata.transitions |= transition(3, 4);

    **************** automate 2 ****************
    automata.initials |= {1,3}; automata.finals |= 3;
    automata.transitions |= transition(1, 'a', 2);
    automata.transitions |= transition(2, 'b', 1);
    automata.transitions |= transition(2, 'b', 3);
    automata.transitions |= transition(3, 'a', 1);
    automata.transitions |= transition(3, 'a', 2);

    automaton a = automata.determine();
    cout << ((a.is_deterministic()) ? "true" : "false") << std::endl;
    // END TEST DETERMINISTIC :
*/
